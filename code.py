{
  (жесткий лимит кодингового блока: (3 часа 5 дней в неделю с утра под брейккорчик/амбиент без уведомлений, в конце коммить че сделал))
  (для обучения юзай документацию -> stackoverflow/duckduckgo)
  (юзай ии как генератор плохого РАНДОМ кода по твоей декомпозиции(чтобы сэкономить время)(быстрее поправить, чем с 0 писать) | (НЕ ЮЗАЙ ИИ ДЛЯ ОБУЧЕНИЯ/ДЕБАГГИНГА/МЫШЛЕНИЯ)
  (понимай 100% кода в своем проекте, понимай 100% контекста информации, которую читаешь, понимай 100% комманд, которые вводишь в терминал)
  (во время этапа кодинга кодь, а во время этапа оптимизации оптимизируй, не смешивай)
  (гугли на английском, если уж переводишь с английского, то переводи только то предложение, которое читаешь, после прочтения его на английском)
  (mobile first)
}
